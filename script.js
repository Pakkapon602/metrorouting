...
var serviceAccount = require("C:/Users/Dell/Desktop/metrodata-804c8-firebase-adminsdk-8r6oz-9ff50bc30c.json");
...
async function loadStationData(filePath) {
    try {
        // const data = fs.readFileSync(filePath, "utf-8");
        await fetch(filePath).then((res) => res.json()).then((data) => {
...
async function enterStation(originName, destinationName) {
    
    await loadStationData("metro.json");
...
// Example usage:
//console.log(enterStation("พญาไท", "บางนา"));
/*
function e(a,b){
    return a+b;
}
...
async function handleUserInput() {
    ...
    if (userMessage.includes("-")) {
        ...
        if (originName && destinationName) {
            ...
            let routeInfo = await enterStation(originName, destinationName);
            console.log('routeInfo ===', routeInfo)
            //const routeInfo = originName + destinationName;
            //const routeInfo = d(originName, destinationName);
